...
# The portrait is drawn at 840px wide but displayed at 400px in the README,
# while the info card is displayed 1:1. Without compensation the portrait's
# window chrome (dots, titlebar text, borders) renders at half the info card's
# apparent size. CHROME scales the chrome -- not the ascii -- so the two cards
# look like the same window.
CHROME = 2.1

PAD = int(20 * CHROME)
TITLEBAR_H = int(30 * CHROME)
STATUS_H = int(30 * CHROME)
DOT_R = 5 * CHROME
DOT_GAP = 16 * CHROME
STROKE = CHROME
RADIUS = 12 * CHROME
...
CURSOR = "#c9d1d9"
STATUS_PROMPT = "Fanfulla@github:~$ whoami "
STATUS_NAME = "Salvatore Arena"
CURSOR_X = (len(STATUS_PROMPT) + len(STATUS_NAME) + 1) * 13 * 0.6 * CHROME

# ---- reveal timing (one-shot; a cursor rasters top -> bottom) -------------
ROW_DUR = 0.075
STAGGER = 0.075      # keep the two equal: that is what makes it read as ONE
                     # cursor rastering down instead of a wave of rows fading in
                     # together. 53 rows -> the portrait finishes in ~4.1s.
...
parts.append(f'<rect width="{CANVAS_W}" height="{CANVAS_H}" rx="{RADIUS:.0f}" fill="url(#bg)"/>')
parts.append(f'<rect x="{STROKE/2}" y="{STROKE/2}" width="{CANVAS_W-STROKE}" height="{CANVAS_H-STROKE}" '
             f'rx="{RADIUS:.0f}" fill="none" stroke="{FRAME}" stroke-width="{STROKE}"/>')

parts.append(f'<line x1="0" y1="{TITLEBAR_H}" x2="{CANVAS_W}" y2="{TITLEBAR_H}" stroke="{FRAME}" stroke-width="{STROKE}"/>')
for i, dotcol in enumerate(["#ff5f56", "#ffbd2e", "#27c93f"]):
    parts.append(f'<circle cx="{PAD + i*DOT_GAP:.0f}" cy="{TITLEBAR_H/2}" r="{DOT_R:.1f}" fill="{dotcol}"/>')
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4*CHROME:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="{12*CHROME:.0f}" text-anchor="middle">Fanfulla@github: ~$ ./portrait.sh</text>')
...
status_y = status_line_y + 19 * CHROME
parts.append(f'<line x1="0" y1="{status_line_y:.1f}" x2="{CANVAS_W}" y2="{status_line_y:.1f}" stroke="{FRAME}" stroke-width="{STROKE}"/>')
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="{13*CHROME:.0f}">'
             f'{STATUS_PROMPT}<tspan fill="{INK}">{STATUS_NAME}</tspan></text>')
parts.append(f'<rect x="{PAD+CURSOR_X:.0f}" y="{status_y-12*CHROME:.1f}" '
             f'width="{8*CHROME:.0f}" height="{14*CHROME:.0f}" fill="{INK}">'
