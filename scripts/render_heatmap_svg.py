...
# GitHub's own 5-step green ramp, unmodified: empty -> densest. No neon top end.
...
# same palette as the portrait and the info card -- the three cards read as one
BG = "#0d1117"
BG2 = "#111722"
FRAME = "#30363d"
MUTED = "#7d8590"
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">Fanfulla@github: ~$ ./contributions --graph</text>')
    ...
    parts.append(f'<text x="{PAD}" y="{ly}" font-size="13" fill="{MUTED}">'
                 ...
                 f'<tspan> contributions across </tspan>'
                 f'<tspan fill="{GREEN}" font-weight="700">{active}</tspan>'
                 f'<tspan> active days</tspan></text>')
    parts.append(f'<text x="{canvas_w - PAD}" y="{ly}" font-size="12" fill="{MUTED}" text-anchor="end">'
                 f'{rng["start"]} &#8594; {rng["end"]}</text>')
    ly += 24
    parts.append(f'<text x="{PAD}" y="{ly}" font-size="13" fill="{MUTED}">avg '
                 f'<tspan fill="{ACCENT}" font-weight="700">{avg}</tspan>'
                 f'<tspan> per active day</tspan></text>')
    parts.append(f'<text x="{canvas_w - PAD}" y="{ly}" font-size="12" fill="{MUTED}" text-anchor="end">'
                 f'busiest day <tspan fill="{ACCENT}" font-weight="700">{best["count"]}</tspan> on {best["date"]}</text>')
